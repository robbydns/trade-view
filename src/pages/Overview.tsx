  ...
  return (
    <div className="space-y-5">
      ...
      <section className="grid gap-3 border-b border-white/10 pb-5 md:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="rounded-md border border-white/10 bg-slate-900/60 p-3 text-xs text-slate-300"><div className="font-semibold text-white">Last Alert</div><div className="mt-2 space-y-1"><div>Coin: {debug?.lastAlertCoin || '-'}</div><div>Time: {debug?.lastAlertTime ? new Date(debug.lastAlertTime).toLocaleTimeString('id-ID') : '-'}</div><div className="break-words">Response: {debug?.lastTelegramResponse || '-'}</div><div className="break-words text-rose-300">Error: {debug?.lastTelegramError || '-'}</div></div></div>
