...
export const savePosition = (position: { symbol: string; quantity: number; entryPrice: number; totalCostUsdt?: number; maxLossPct: number }) => request<PositionRecord>('/portfolio', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(position) })
