...
const isQualified = (ticker: MarketTicker) =>
  ticker.score >= 88 &&
...
const updatePersistedStatuses = (tickers: MarketTicker[]) => {
  ...
  store.mutate((draft) => {
    for (const record of draft.signalHistory) {
      ...
      const nearestAfter = (hours: number) => {
        const target = recordTime + hours * 60 * 60 * 1000
        const tolerance = 20 * 60 * 1000
        const nearest = symbolSnapshots.reduce<SnapshotRecord | null>((best, snapshot) => {
...
export const getAnalytics = () => {
  ...
  const average = (items: SignalHistoryRecord[], key: 'gain1hPct' | 'gain4hPct' | 'gain24hPct') => items.length ? Number((items.reduce((sum, item) => sum + Number(item[key] || 0), 0) / items.length).toFixed(2)) : 0
...
const evaluatePosition = async (position: PositionRecord): Promise<PositionEvaluation> => {
  ...
  let outlook: PositionEvaluation['outlook'] = 'PELUANG NAIK TERBATAS'
...
export const upsertPosition = (input: { symbol: string; quantity: number; entryPrice?: number; totalCostUsdt?: number; maxLossPct?: number }) => {
  ...
  if (!symbol || !Number.isFinite(quantity) || quantity <= 0 || !Number.isFinite(averageEntryPrice) || averageEntryPrice <= 0 || !Number.isFinite(totalCostUsdt) || totalCostUsdt <= 0 || !Number.isFinite(maxLossPct) || maxLossPct <= 0 || maxLossPct > 100) throw new Error('Jumlah coin, harga entry, total modal, dan batas rugi wajib diisi dengan angka valid.')
  const existing = store.get().positions.find((item) => item.symbol === symbol)
  const now = new Date().toISOString()
  const record: PositionRecord = { id: existing?.id || id(), symbol, quantity, totalCostUsdt: Number(totalCostUsdt.toFixed(8)), averageEntryPrice: Number(averageEntryPrice.toFixed(8)), maxLossPct, createdAt: existing?.createdAt || now, updatedAt: now }
