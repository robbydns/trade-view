...
const isValidChatId = (value) => /^-?\d+$/.test(value);
const isValidToken = (value) => /^\d+:[A-Za-z0-9_-]+$/.test(value);
let settings = {
...
export const updateTelegramSettings = (next) => {
...
export const sendTelegramAlert = async (message, automatic = false) => {
    if (automatic && !settings.enabled)
        throw new Error('Telegram alert dinonaktifkan.');
    ...
    throw new Error(`Telegram gagal setelah 3 percobaan: ${telegramDescription || lastError?.message || 'unknown error'}`);
};
