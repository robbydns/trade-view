...
const applyHigherTimeframeConfirmation = (signal, candles1h, candles1d) => {
    const trend = (candles) => {
    ...
    if (trend1h === 'BULLISH') {
        score += 5;
        reasons.push('Konfirmasi tren 1H bullish');
    }
    if (trend1d === 'BULLISH') {
        score += 8;
        reasons.push('Konfirmasi tren 1D bullish');
    }
    if (trend1h === 'BEARISH') {
        score -= 12;
        reasons.push('Momentum 15m melawan tren 1H bearish');
    }
    if (trend1d === 'BEARISH') {
        score -= 18;
        reasons.push('Counter trend: arah besar 1D masih bearish');
    }
    score = Math.max(0, Math.min(100, score));
    let label = score >= 90 ? 'STRONG BUY' : score >= 80 ? 'BREAKOUT' : score >= 65 ? 'BUY WATCH' : score >= 50 ? 'WAIT' : 'AVOID';
    if (signal.isOverheated)
        label = 'OVERHEATED';
...
const buildSignal = (input) => {
    ...
    if (input.priceChange1h >= 0.5 && input.priceChange1h <= 12) {
        score += 8;
        reasons.push(`Momentum 1 jam sehat di ${input.priceChange1h}%`);
    }
    if (latest.close > latest.open && closeNearHigh) {
        score += 7;
        reasons.push('Candle ditutup dekat high tanpa rejection besar');
    }
    if (liquidEnough) {
        score += 5;
        reasons.push('Likuiditas volume 24 jam memadai');
    }
    ...
    if (input.priceChange24h < -5) {
        score -= 15;
        reasons.push('Trend 24h masih negatif');
    }
    if (input.priceChange1h < 0) {
        score -= 15;
        reasons.push('Momentum 1 jam masih negatif');
    }
    if (input.distanceFromMa10Pct > 12) {
        score -= 20;
        reasons.push('Harga terlalu jauh dari MA10');
    }
    if (input.distanceFromMa10Pct < -2) {
        score -= 15;
        reasons.push('Harga masih berada cukup jauh di bawah MA10');
    }
    if (input.rsi15m > 75) {
        score -= 15;
    ...
    if (!liquidEnough) {
        score -= 20;
        reasons.push('Volume 24 jam terlalu rendah untuk sinyal berkualitas');
    }
    if (!orderlyVolume) {
        score -= 15;
        reasons.push('Lonjakan volume terlalu ekstrem dan rawan distribusi');
    }
    if (upperWickRisk) {
        score -= 20;
        reasons.push('Wick atas besar menunjukkan rejection');
    }
    score = Math.max(0, Math.min(100, score));
        ...
        try {
            ...
            if (signal.score >= 65) {
                try {
                    ...
                    const candles1h = candles1hResponse.data.map(parseCandle);
                    const candles1d = candles1dResponse.data.map(parseCandle);
                    if (candles1h.length >= 50 && candles1d.length >= 50)
                        signal = applyHigherTimeframeConfirmation(signal, candles1h, candles1d);
                }
                catch {
